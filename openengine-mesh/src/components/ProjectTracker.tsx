...
export const ProjectTracker: React.FC<ProjectTrackerProps> = ({
  ...
  const [project] = useState<ProjectOverview>({
    id: 'proj-zero-petri',
    name: 'OpenEngine Zero-Petri Mesh & Gated Workflows',
    repo: 'foxlight/zero-petri',
    branch: 'main',
    description:
      'Distributed P2P agent orchestrator packaged with Tauri v2 across Linux, Android, and Web, backed by gated autonomous GitHub deliveries.',
    milestones: [
      {
        id: 'ms-01',
        title: 'Mesh Fabric & Discovery',
        phase: 'Phase 01',
        description: 'Tailscale WireGuard mesh sync, live peer status, and encrypted heartbeat routing.',
        status: 'completed',
        progress: 100,
      ...
      {
        id: 'ms-02',
        title: 'Hardware Role Negotiation',
        phase: 'Phase 02',
        description: 'NVIDIA RTX VRAM auto-detection, Omarchy host daemon, and mobile thin client.',
        status: 'completed',
        progress: 100,
        taskIds: ['tsk-103'],
      },
      {
        id: 'ms-03',
        title: 'Autonomous Verification & Repair',
        phase: 'Phase 03',
        description: 'Parallel acceptance testing, code security review, and bounded 3-turn auto-repair.',
        status: 'in_progress',
        progress: 85,
        taskIds: ['tsk-104'],
      },
      {
        id: 'ms-04',
        title: 'Gated Signoff & GitHub Delivery',
        phase: 'Phase 04',
        description: '1-tap signoff on Android/Omarchy, signed git commits, PR creation, and CI merge.',
        status: 'in_progress',
        progress: 75,
        taskIds: ['tsk-105'],
      },
  ...
  const getStatusBadge = (status: TaskLifecycleStatus) => {
    switch (status) {
      case 'completed':
        return (
          <span className="flex items-center space-x-1.5 text-[#a3a3a3] font-mono text-[10px]">
      ...
      case 'gated':
        return (
          <span className="flex items-center space-x-1.5 text-[#e5e5e5] font-mono text-[10px] animate-pulse">
      ...
      case 'verifying':
        return (
          <span className="flex items-center space-x-1.5 text-[#737373] font-mono text-[10px]">
      ...
      default:
        return (
          <span className="flex items-center space-x-1.5 text-[#525252] font-mono text-[10px]">
  ...
  return (
    <div className="flex flex-col h-full w-full bg-[#070707] text-[#e0e0e0] font-sans select-none overflow-y-auto">
      ...
      <div className="border-b border-[#1c1c1c] p-6 bg-[#0a0a0a]/90 backdrop-blur-md">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="space-y-1.5">
            <div className="flex items-center space-x-2 text-xs text-[#737373] font-mono tracking-wide">
          ...
          <div className="flex items-center gap-5 border border-[#1f1f1f] bg-[#0c0c0c] px-4 py-2.5 rounded-lg text-xs">
            <div className="flex flex-col">
              <span className="text-[10px] text-[#525252] uppercase font-mono">Completion</span>
              <span className="text-sm font-mono font-medium text-[#e5e5e5]">
                {Math.round((project.metrics.completedTasks / project.metrics.totalTasks) * 100)}%
              </span>
            </div>
            <div className="w-[1px] h-6 bg-[#1f1f1f]" />
            <div className="flex flex-col">
              <span className="text-[10px] text-[#525252] uppercase font-mono">Pass Rate</span>
              <span className="text-sm font-mono font-medium text-[#e5e5e5]">
            ...
            <div className="w-[1px] h-6 bg-[#1f1f1f]" />
            <div className="flex flex-col">
              <span className="text-[10px] text-[#525252] uppercase font-mono">Gated Signoff</span>
              <span className="text-sm font-mono font-medium text-[#e5e5e5]">
      ...
      <div className="border-b border-[#1c1c1c] px-6 py-4 bg-[#090909]">
        <div className="text-[10px] uppercase font-mono text-[#525252] tracking-wider mb-3 flex items-center space-x-1.5">
      ...
      <div className="flex-1 p-6 space-y-4">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs">
          <div className="flex items-center space-x-2">
            {['all', 'active', 'gated', 'completed'].map((tab) => (
              <button
                ...
                className={`px-3 py-1.5 rounded-md font-mono text-[11px] capitalize transition-colors ${
                  filterStatus === tab
          ...
          <div className="flex items-center space-x-2 bg-[#0c0c0c] border border-[#1c1c1c] px-2.5 py-1.5 rounded-md w-full sm:w-64">
            ...
            <input
              ...
              className="bg-transparent focus:outline-none text-[11px] text-[#e0e0e0] placeholder-[#525252] w-full font-mono"
            />
          </div>
        </div>
        ...
        <div className="border border-[#1c1c1c] rounded-lg overflow-hidden bg-[#0a0a0a]">
          <div className="grid grid-cols-12 px-4 py-2.5 border-b border-[#1c1c1c] text-[10px] font-mono uppercase text-[#525252] tracking-wider bg-[#0d0d0d]">
            ...
            {filteredTasks.map((task) => (
              ...
              >
                ...
                <div className="col-span-4 sm:col-span-3 space-y-0.5">
                  <div className="font-mono text-[10px] text-[#737373] flex items-center space-x-1.5">
                    <span>{task.id}</span>
                    <span>·</span>
                    <span className="text-[#a3a3a3]">{task.component}</span>
                  </div>
                  {task.commitHash && (
                    <div className="flex items-center space-x-1 text-[10px] text-[#525252] font-mono">
                ...
                {/* Agent */}
                <div className="hidden sm:block sm:col-span-2 text-[11px] text-[#737373] font-mono">
                  {task.assignedAgent}
                </div>
                ...
                <div className="col-span-4 sm:col-span-3 flex items-center justify-end space-x-3">
                  ...
                  {task.status === 'gated' && task.runId && (
                    <button
                      ...
                      className="px-2.5 py-1 rounded bg-[#222222] hover:bg-[#2b2b2b] text-[#f5f5f5] text-[10px] font-mono border border-[#333] transition-colors"
                    >
                      Signoff
                    </button>
                  )}
        ...
        {/* Telemetry Footer */}
        <div className="flex flex-col sm:flex-row items-center justify-between text-[11px] font-mono text-[#525252] px-2 py-3 border-t border-[#171717] gap-2">
