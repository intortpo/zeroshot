...
fn write_process_error(error: &ProcessError) {
    let error_format = std::env::var(ERROR_FORMAT_ENV)
        .or_else(|_| std::env::var("ZEROSHOT_ERROR_FORMAT"));
    if error_format.as_deref() == Ok(JSON_ERROR_FORMAT) {
