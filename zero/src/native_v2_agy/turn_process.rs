...
use super::transcript::AgyTranscript;
use super::AgyAttempt;

pub(super) enum AgyProcessStart {
...
pub(super) fn failed_before_start(
    ...
    Ok(AgyProcessStart::Failed(AgyAttempt::process_failure(
        error.to_string(),
    )))
}
...
pub(super) async fn open(
    ...
    Ok(AgyProcessStart::Ready(process))
}

pub(super) async fn finish_process(
    process: &mut ProcessSession,
    prompt: &[u8],
    mut transcript: AgyTranscript,
    control: &DriverControl,
...
async fn finish_process_completion(
    ...
    let failure =
        match process_failure_detail(&output, control.is_cancelled(), false) {
...
pub(super) async fn release_failure(
    ...
    append_completion_detail(&mut diagnostic, completion)?;
    Ok(AgyAttempt::process_failure(diagnostic))
}
