...
use crate::native_v2_agy::{AgyAdapter, AgyAdapterConfig, AgyAdapterConfigError};
...
fn build_candidate(
    ...
    match config.harness {
        ...
        NativeV2HarnessConfig::Agy(config) => {
            let agent = Arc::new(match placement {
                ProcessPlacement::Capsule => AgyAdapter::new(config)?,
                ProcessPlacement::Local => AgyAdapter::new_local(config)?,
            });
            assemble_runner(admitted, agent.clone(), agent, delivery)
        }
    }
...
fn validate_config(
    ...
    let workspace_matches = match (&admitted.runtime, &config.harness) {
        ...
        (RuntimePlan::Agy { provider, .. }, NativeV2HarnessConfig::Agy(harness))
            if provider == &harness.provider =>
        {
            harness.workspace == config.delivery.workspace
        }
