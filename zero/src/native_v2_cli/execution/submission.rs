...
use openengine_cluster_protocol::{
    AgyProvider, ClaudeProvider, CodexProvider, DeclaredConnections, DeclaredEnvironment,
...
const KNOWN_INCOMPATIBLE_HARNESS_PROVIDER_PAIRS: &[(UniformHarness, UniformProvider)] = &[
    ...
    (UniformHarness::Codex, UniformProvider::Gemini),
    (UniformHarness::Codex, UniformProvider::Vertex),
    (UniformHarness::Claude, UniformProvider::OpenAi),
    (UniformHarness::Claude, UniformProvider::Gemini),
    (UniformHarness::Claude, UniformProvider::Vertex),
    (UniformHarness::Agy, UniformProvider::OpenAi),
    (UniformHarness::Agy, UniformProvider::Anthropic),
    (UniformHarness::Agy, UniformProvider::OpenRouter),
    (UniformHarness::Agy, UniformProvider::Bedrock),
];
...
impl UniformRuntimePlan {
    ...
    fn into_runtime_plan(
        ...
        match (harness, self.provider) {
            (UniformHarness::Agy, UniformProvider::Gemini) => Ok(RuntimePlan::Agy {
                provider: AgyProvider::Gemini,
                size: self.size,
                nodes,
            }),
            (UniformHarness::Agy, UniformProvider::Vertex) => Ok(RuntimePlan::Agy {
                provider: AgyProvider::Vertex,
                size: self.size,
                nodes,
            }),
...
fn default_connections(provider: UniformProvider) -> Result<DeclaredConnections, NativeV2CliError> {
    let (key, names): (&str, &[&str]) = match provider {
        UniformProvider::Gemini => ("gemini", &["GEMINI_API_KEY"]),
        UniformProvider::Vertex => ("vertex", &[
