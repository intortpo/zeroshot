...
/// Run multi-agent graphs locally or on named Zero targets.
///
...
A runtime declares a connection key and the exact environment fields it needs. Zero injects
...
EXAMPLES
  zero connection list
  zero connection list --target prod --scope org
  zero connection set openrouter --field OPENROUTER_API_KEY
  zero connection set openrouter --target prod --field OPENROUTER_API_KEY"#)]
...
#[command(after_long_help = r#"RUNTIME CONFIGURATION
    ...
    Use `zero template show TEMPLATE` to inspect node names. With --pr or --ship, omit the
    template-owned delivery binding.

    --uniform-runtime-config requires harness, provider, and model. It accepts optional size,
    effort, sessionScope, and connections fields without nodes. Zero expands that agent binding
