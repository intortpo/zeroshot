...
use crate::native_v2_agy::{AgyAdapterConfig, AgyAdapterConfigError, AgyProcessEnvironment};
...
pub enum LocalCompositionError {
    ...
    Storage,
    #[error(transparent)]
    Agy(#[from] AgyAdapterConfigError),
    #[error(transparent)]
...
pub fn build_local_process_candidate(
    ...
    let harness = match &admitted.runtime {
        ...
        RuntimePlan::Agy { provider, .. } => {
            let executable = std::env::var("ZERO_AGY_EXECUTABLE")
                .or_else(|_| std::env::var("ZEROSHOT_AGY_EXECUTABLE"))
                .unwrap_or_else(|_| "agy".to_owned());
            NativeV2HarnessConfig::Agy(AgyAdapterConfig {
                provider: *provider,
                executable,
                prefix_arguments: Vec::new(),
                workspace: workspace.to_owned(),
                runtime_home: runtime_home.clone(),
                local_user_home: local_home,
                base_environment: local_agy_environment(&search_path)?,
                turn_timeout: LOCAL_TURN_TIMEOUT,
                process_pool,
            })
        }
...
fn local_agy_environment(
    search_path: &str,
) -> Result<AgyProcessEnvironment, AgyAdapterConfigError> {
    let mut base_environment = BTreeMap::from([("PATH".to_owned(), search_path.to_owned())]);
    for name in ["LANG", "LC_ALL", "TERM", "TMPDIR"] {
        if let Ok(value) = std::env::var(name) {
            base_environment.insert(name.to_owned(), value);
    ...
    AgyProcessEnvironment::new(base_environment)
}
