...
const CREDENTIAL_STORE_ENV: &str = "ZERO_CREDENTIAL_STORE";
const LEGACY_CREDENTIAL_STORE_ENV: &str = "ZEROSHOT_CREDENTIAL_STORE";
...
impl LinuxTargetCredentialStore {
    pub(super) fn from_environment(directory: PathBuf) -> Result<Self, TargetAuthorityError> {
        let env_val = std::env::var_os(CREDENTIAL_STORE_ENV)
            .or_else(|| std::env::var_os(LEGACY_CREDENTIAL_STORE_ENV));
        Ok(Self {
            system: Arc::new(KeyringTargetCredentialStore),
            file: PrivateFileTargetCredentialStore::new(directory),
            forced: parse_preference(env_val.as_deref())?,
...
fn parse_preference(value: Option<&OsStr>) -> Result<Option<Backend>, TargetAuthorityError> {
    ...
    match value.to_str() {
        ...
        _ => Err(authority_error(
            "ZERO_CREDENTIAL_STORE must be auto, system, or file",
        )),
    }
}
